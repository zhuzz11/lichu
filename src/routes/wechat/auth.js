...
var checkAccessToken = function(req, res, next) {
	...
	redisUtil.get("access_token").then(function(data) {
		//获取到值--往下传递  
		if (data) {
		...
		if (!data.expires_in) {
			...
			redisUtil.set("access_token", data.access_token, 7180).then(function(result) {
				if (result == 'OK') {
...
var checkTicket = function(req, res, next) {
	...
	//根据token从redis中获取access_token  
	redisUtil.get("jsapi_ticket").then(function(data) {
		//获取到值--往下传递  
		if (data) {
			return Promise.resolve(data);
		} else {//没获取到值--从微信服务器端获取,并往下传递  
		...
		if (!data.expires_in) {
			...
			redisUtil.set("jsapi_ticket", data.ticket, 7180).then(function(result) {
				if (result == 'OK') {
